...
function getPositions($es_n) {
  ...
  if(mysqli_num_rows($query_post) > 0) {
    ...
    while($row = mysqli_fetch_assoc($query_post)) {
      ...
      print_r('<div class="col position" id="'.$ep_n.'" election="'.$es_n.'"><span class="del-btn" ec_n="" es_n="'.$es_n.'" ep_n="'.$ep_n.'"><i class="fas fa-trash-alt color-1"></i></span>'.$ep_n.'</div>');
    }
    print_r('
      </div>');
  } else {
...
function getElections() {
  ...
  if($count > 0) {
    while($row = mysqli_fetch_assoc($query)) {
      ...
      print_r('<div class="tab-pane-baby col-10 offset-1"><span class="del-btn" ec_n="" ep_n="" es_n="'.$election.'"><i class="fas fa-trash-alt color-2"></i></span><h5>'.$election.'</h5>');
